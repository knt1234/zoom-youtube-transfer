...
> 💡 Zoomは一度に約1か月分の録画しか返さないため、1か月を超える期間を指定した場合は自動的に30日ごとに分割して取得します。長い期間をまとめて指定しても取りこぼしはありません。

---

## セキュリティについて

- **APIキーや認証ファイルはGitHubに公開されません** — YouTube連携に使うファイル（`client_secrets.json`）や認証情報は、GitHubにはアップロードされない設定になっています。あなたのパソコン内にのみ存在します。
- **ZoomのAPIキーは自分だけが見られます** — ColabのシークレットはGoogleアカウントに紐付けて保存されるため、他の人からは一切見えません。
